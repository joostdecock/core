<?php
/** Freesewing\Patterns\Core\HugoHoodie class */
...
class HugoHoodie extends BrianBodyBlock
{
    ...
    */
    ...
    public function draftSleeve($model)
    {
        ...
        foreach ($load as $i) {
            $p->addPoint( 200+$i, $p->shift(200+$i, $angle, $distance*1));
        }
        // Rotate in place
        $angle = -1 * ($p->angle(1, 208) - 90);
        foreach ($load as $i) {
        ...
        $p->newPath('seamline', $path, ['class' => 'fabric']);
        //$p->newPath('seamline', 'M 31 L 31', ['class' => 'fabric']);
