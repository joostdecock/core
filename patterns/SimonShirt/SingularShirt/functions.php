...
function tweakSleevecap()
{
    ...
    if ($SLEEVECAPITERATION > 144) {
        msg("      Iteration $SLEEVECAPITERATION, sleevecap length is $delta mm off. I'm not happy, but it will have to do.");
    } else {
        msg("      Iteration $SLEEVECAPITERATION, sleevecap length is $delta mm off. I'm happy.");
    }
    dmsg();
...
function fitSleevecap($delta)
{
  ...
  if ($SLEEVECAPITERATION > 18) {
      if ($delta > 0) {
          $SLEEVECAPFACTOR = $SLEEVECAPFACTOR * 0.9;
      } elseif ($delta < 0) {
          $SLEEVECAPFACTOR = $SLEEVECAPFACTOR * 1.1;
      }
  } elseif ($SLEEVECAPITERATION > 10) {
      if ($delta > 0) {
          $SLEEVECAPFACTOR = $SLEEVECAPFACTOR * 0.95;
      } elseif ($delta < 0) {
          $SLEEVECAPFACTOR = $SLEEVECAPFACTOR * 1.05;
      }
  } else {
      if ($delta > 0) {
          $SLEEVECAPFACTOR = $SLEEVECAPFACTOR * 0.99;
      } elseif ($delta < 0) {
          $SLEEVECAPFACTOR = $SLEEVECAPFACTOR * 1.01;
      }
  }
    buildSleeve();
}
function tweakCollaropening()
{
    ...
    if ($COLLAROPENINGITERATION > 149) {
        msg("      Iteration $COLLAROPENINGITERATION, collar opening length is $delta mm off. I'm not happy, but it will have to do.");
    } else {
        msg("      Iteration $COLLAROPENINGITERATION, collar opening length is $delta mm off. I'm happy.");
    }
    dmsg();
...
function fitCollaropening($delta)
{
  ...
  // Adjusting sleevecap factor
  if ($delta > 0) {
      $COLLAROPENINGFACTOR = $COLLAROPENINGFACTOR * 0.97;
  } elseif ($delta < 0) {
      $COLLAROPENINGFACTOR = $COLLAROPENINGFACTOR * 1.03;
  }
    buildBody();
}
...
function fitCollarstand($delta)
{
  ...
  if ($delta > 0) {
      $COLLARSTANDFACTOR = $COLLARSTANDFACTOR * 0.98;
  } elseif ($delta < 0) {
      $COLLARSTANDFACTOR = $COLLARSTANDFACTOR * 1.02;
  }
    buildCollarstand();
}
function tweakCollarstand()
{
    ...
    if ($COLLARSTANDITERATION > 149) {
        msg("      Iteration $COLLARSTANDITERATION, collar stand length is $delta mm off. I'm not happy, but it will have to do.");
    } else {
        msg("      Iteration $COLLARSTANDITERATION, collar stand length is $delta mm off. I'm happy.");
    }
    dmsg();
...
function fitCollar($delta)
{
  ...
  if ($delta > 0) {
      $COLLARFACTOR = $COLLARFACTOR * 0.98;
  } elseif ($delta < 0) {
      $COLLARFACTOR = $COLLARFACTOR * 1.02;
  }
    buildCollar();
}
function tweakCollar()
{
    ...
    if ($COLLARITERATION > 149) {
        msg("      Iteration $COLLARITERATION, collar length is $delta mm off. I'm not happy, but it will have to do.");
    } else {
        msg("      Iteration $COLLARITERATION, collar length is $delta mm off. I'm happy.");
    }
    dmsg();
...
function buildFrontAndBackBlock()
{
  ...
  $backarmpitchY = py(15) - 110;
    if ($backarmpitchY == py(12)) {
        addPoint(20, px(12), py(12), 'Back pitch point');
    } // Smack between curves
  elseif ($backarmpitchY < py(12)) {
      addPntAr(20, arcYsect(12, 121, 161, 16, $backarmpitchY), 'Back pitch point');
  } // Top curve
  elseif ($backarmpitchY > py(12)) {
      addPntAr(20, arcYsect(18, 181, 122, 12, $backarmpitchY), 'Back pitch point');
  } // Bottom curve
  // Need to figure out what curve the front pitch point will intersect with
  $frontarmpitchY = py(-15) - 80;
    if ($frontarmpitchY == py(-12)) {
        addPoint(-20, px(-12), py(-12), 'Front pitch point');
    } // Smack between curves
  elseif ($frontarmpitchY < py(-12)) {
      addPntAr(-20, arcYsect(-12, -121, -161, -16, $frontarmpitchY), 'Front pitch point');
  } // Top curve
  elseif ($frontarmpitchY > py(-12)) {
      addPntAr(-20, arcYsect(-18, -181, -122, -12, $frontarmpitchY), 'Front pitch point');
  } // Bottom curve
...
function buildSleeve()
{
  ...
  if (getp('CUFF_STYLE') < 4) {
      $cuffwidth = getm('WristCircumference') + getp('CUFF_EASE') + 20;
  } elseif (getp('CUFF_STYLE') == 6) {
      $cuffwidth = getm('WristCircumference') + getp('CUFF_EASE') + 30;
  } else {
      $cuffwidth = getm('WristCircumference') + getp('CUFF_EASE') + 30 - getp('CUFF_LENGTH') / 2;
  }
...
function mmpPoints($part = false, $model = false)
{
    ...
    case 'FrontRight':
      ...
      if (getp('BUTTON_PLACKET_STYLE') == 1) {
          $width2 = $width * 3;
      } // Classic placket
      else {
          $width2 = $width * 5;
      ...
      if (getp('BUTTON_PLACKET_STYLE') == 2) {
          addPntAr(2046, pShift(-4, 0, $width * 3));
      }
    ...
    case 'FrontLeft':
      ...
      if (getp('BUTTONHOLE_PLACKET_TYPE') == 1) {
          addMarker(3000, 'buttonhole', array('rotate' => 90));
      }
      for ($i = 1; $i < getp('FRONT_BUTTONS'); ++$i) {
          $pid = 3000 + $i;
          addPntAr($pid, pShift(3000, 90, $btndist * $i), 'Button');
    ...
    case 'Yoke':
      ...
      if (getp('SPLIT_YOKE')) {
          addPoint(40050, px(-4002), py(4005));
      } else {
          addPoint(40050, px(4005), py(4005));
      }
    ...
    case 'Sleeve':
      ...
      if ($lenq12 == $len) {
          addPoint(801000, pxy(801), 'Front pitch point');
      } // Pitch exactly one borden Q1 and Q2
      elseif ($lenq12 > $len) { // Pitch in Q1
      ...
      if ($len == $lenq14) {
          addPntAr(43000, pxy(43), 'Shoulder point');
      } // Shoulder point exactly where intended
      elseif ($len > $lenq14) { // Shoulder point more towards the back
      ...
      if ($lenq12 == $len) {
          addPoint(701000, pxy(701), 'Front pitch point');
      } // Pitch exactly one borden Q1 and Q2
      elseif ($lenq12 > $len) { // Pitch in Q1
...
function mmpTitles($part = false, $model = false)
{
    ...
    case 'Yoke':
      if (getp('SPLIT_YOKE')) {
          addTitle('4', 'Yoke|Cut 4', 40050, $part, $model);
      } else {
          addTitle('4', 'Yoke|Cut 2', 40050, $part, $model);
      }
    break;
    case 'Sleeve':
...
function mmpPaths($part = false)
{
    ...
    case 'FrontRight':
     ...
     if (getp('BUTTON_PLACKET_TYPE') == 2) {
      ...
      if ($placketfold) {
          addPath($placketfold, 'foldline');
      }
      if ($plackethelp) {
          addPath($plackethelp, 'helpline');
      }
      addPath($acrossback, 'helpline');
      addPath($chestline, 'helpline');
      addPath($waistline, 'helpline');
    ...
    case 'FrontLeft':
      ...
      if (getp('BUTTONHOLE_PLACKET_TYPE') == 1) { // Grown on
        $hem .= 'L 6660 ';
          if (getp('BUTTONHOLE_PLACKET_STYLE') == 1) {
              $path = 'M 17 L 4107 L 4007 ';
          } // Classic placket
        else {
            $path = 'M 17 L 41086 C 41084 41085 41083 C 41088 41087 41089 L 4107 L 4007 ';
        } // Seamless
      } else { // Sewn on
        $hem .= 'L 4008';
          $path = 'M 4008 ';
    ...
    case 'ButtonholePlacket':
      if (getp('BUTTONHOLE_PLACKET_STYLE') == 1) {
          $path = 'M 4108 C 41082 41081 17 L 4107 L 4007 L 4008 z';
      } // Classic placket
      else {
          $path = 'M 4108 C 41082 41081 4100 L 41086 C 41084 41085 41083 C 41088 41087 41089 L 4107 L 4007 L 4008 z';
      } // Seamles
    ...
    case 'Back':
      if (getp('HEM_STYLE') == 1) {
          ...
          $hem = 'M -6663 C -6664 -6665 -6666 C -6667 -6668 -6669 L 6660 L 6669 C 6668 6667 6666 C 6665 6664 6663';
      }
      addPath($hem, 'hemmed');
    ...
    break;
    case 'Yoke':
      if (getp('SPLIT_YOKE') == 1) {
          $path = 'M 1 C -101 -10 -10 L -16 C -161 -121 -20 L 11 z';
      } else {
          $path = 'M 1 C -101 -10 -10 L -16 C -161 -121 -20 L 20 C 121 161 16 L 10 C 10 101 1 z';
      }
      addPathWithSa($path);
      if (getp('SPLIT_YOKE') == 1) {
          $grainhelp = 'M 11 L -4002 M -4004 L 1 M 4008 L 4005';
      } else {
          $grainhelp = 'M 1 L 1011 M 4002 L 1011 L -4002 M -4004 L 1 L 4004 M 4007 L 4008';
      }
...
function emailPatternInfo()
{
  ...
  .','.getp('SLEEVE_PLACKET_LENGTH') / 10;
    if ($mail == 'joost@decock.org') {
        if (!$handle = fopen($csv, 'a')) {
            dlog("Could not open $csv");
            exit;
        }
        if (fwrite($handle, $postman) === false) {
            dlog("Could not write to $csv");
            exit;
        }
        fclose($handle);
        dlog($cmd);
